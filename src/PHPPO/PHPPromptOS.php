...
class PHPPromptOS{
  ...
  function setup(){
    ...
    foreach ($Install_file_path_array as $key => $value) {
    	$installPath = $this->path . $key;
    	$resourcePath = $this->path . $value;
  ...
  function __construct(){
    ...
    $first_time_boot = !file_exists($this->path . "\\root\bin\\" . 'systemdefinedvars.dat');
    if ($first_time_boot) {
    	$this->setup();
    }
