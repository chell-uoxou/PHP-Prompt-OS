...
function bootSystem($tipe){
	...
	if ($tipe != "script") {
		...
		$system->sendMessage("\x1b[38;5;227m  :+:    :+: :+:    :+: :+:    :+: :+:    :+: :+:    :+:  PHP Prompt OS");
		$system->sendMessage("\x1b[38;5;227m  +:+    +:+ +:+    +:+ +:+    +:+ +:+    +:+ +:+    +:+     made by chell rui.");
		$system->sendMessage("\x1b[38;5;227m  +#++:++#+  +#++:++#++ +#++:++#+  +#++:++#+  +#+    +:+        version \x1b[38;5;83m$version");
		$system->sendMessage("\x1b[38;5;227m  +#+        +#+    +#+ +#+        +#+        +#+    +#+             Running in \x1b[38;5;83mPHP\x1b[38;5;214m7");
		$system->sendMessage("\x1b[38;5;227m  #+#        #+#    #+# #+#        #+#        #+#    #+# ");
		$system->sendMessage("\x1b[38;5;227m  ###        ###    ### ###        ###         ########  ");
		$system->sendMessage("");
		$system->sendMessage("\x1b[38;5;214m┏╋‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━‥━━╋┓");
		...
		global $tipe;
	  	...
	  	$system->sendMessage("\x1b[38;5;227mPHP Prompt OS\x1b[38;5;145m version \x1b[38;5;83m$version");
...
function readySetup($tipe){
		...
		global $version;
		$userfilepath = rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "user.json";
		...
		$file_name = rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "README.txt";
		if( !file_exists($file_name) ){
			touch( $file_name );
		}
		chmod( $file_name, 0666 );

		$logfilepath = rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "logs";
		if (!file_exists($logfilepath)) {
			mkdir($logfilepath);
			$system->sendMessage("ログ出力用フォルダを生成しました。:[" . $logfilepath .  "]");
		 }

		 $scriptsfilepath = rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "scripts";
		 if (!file_exists($scriptsfilepath)) {
			 mkdir($scriptsfilepath);
			 $system->sendMessage("スクリプト、ユーザー定義コマンド保存用フォルダを生成しました。:[" . $scriptsfilepath .  "]");
		 }

		$logfilepath = rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "logs" . "\\" . date('Y_m_d') . ".log";
	 	if (!file_exists($logfilepath)) {
	 	touch($logfilepath);
		...
		chmod( $logfilepath, 0666 );
	 	...
	 	$system->systemFileOpen(rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "logs" . "\\" . date('Y_m_d') . ".log");
		global $writeData;
		$writeData = fopen(rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "logs" . "\\" . date('Y_m_d') . ".log",'a');
		$pr_info = "INFO";
	 	$pr_thread = "File";
	 	$LICENSE = fopen(rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . 'LICENSE.txt', "w");
...
You should have received a copy of the GNU General Public License along with this program. If not, see httpwww.gnu.orglicenses.");
		...
		$LICENSE = fopen(rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . 'README.txt', "w");
	...
	■■■ＲＥＬＥＡＳＥ　ＮＯＴＥ──────────────────────────────
		...
		$file_name = rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . 'config.ini';
		readScripts();
		if(!file_exists($file_name)){
			touch($file_name);
	...
	function readScripts(){
		...
		$aryScripts = array();
			// ディレクトリのパスを記述
		$dir = rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "scripts" ;

		// ディレクトリの存在を確認し、ハンドルを取得
		$i = 0;
		$handle = opendir(rtrim(dirname(__FILE__),"\src\PHPPO") . "\\" . "scripts" . "\\");
...
function standbyTipe(){
	...
	while (True) {
		...
		$pr_time = date('A-H:i:s');
		if ($echoFunc != "off") {
			echo PHP_EOL . "\x1b[38;5;83m" . "[{$pr_time}]" . "\x1b[38;5;87m" . " [{$pr_thread} Thread/{$pr_info}]" . "\x1b[38;5;227m>";
		}else {
			echo "\x1b[38;5;227m" . PHP_EOL;
		}
		$stanby = false;
